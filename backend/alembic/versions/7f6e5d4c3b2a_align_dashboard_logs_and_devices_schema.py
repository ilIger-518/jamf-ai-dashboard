...
def upgrade() -> None:
    ...
    if "dashboard_logs" in tables:
        ...
        if "user_id" in dashboard_columns and not dashboard_columns["user_id"]["nullable"]:
            op.alter_column("dashboard_logs", "user_id", existing_type=postgresql.UUID(as_uuid=True), nullable=True)

        if "resource_type" in dashboard_columns:
            conn.execute(sa.text("UPDATE dashboard_logs SET resource_type = category WHERE resource_type IS NULL"))
            op.alter_column("dashboard_logs", "resource_type", existing_type=sa.String(length=255), nullable=True)
    ...
    if "devices" in tables:
        ...
        if "model_identifier" not in device_columns:
            missing_columns.append(sa.Column("model_identifier", sa.String(length=64), nullable=True))
        ...
        if "is_managed" not in device_columns:
            missing_columns.append(sa.Column("is_managed", sa.Boolean(), nullable=False, server_default=sa.text("false")))
        if "is_supervised" not in device_columns:
            missing_columns.append(sa.Column("is_supervised", sa.Boolean(), nullable=False, server_default=sa.text("false")))
        if "last_contact" not in device_columns:
            missing_columns.append(sa.Column("last_contact", sa.DateTime(timezone=True), nullable=True))
        if "last_enrollment" not in device_columns:
            missing_columns.append(sa.Column("last_enrollment", sa.DateTime(timezone=True), nullable=True))
        ...
        if "ix_devices_udid" not in device_indexes and "udid" in {col["name"] for col in inspector.get_columns("devices")}:
        ...
        if "ix_devices_serial_number" not in device_indexes and "serial_number" in {col["name"] for col in inspector.get_columns("devices")}:
            op.create_index("ix_devices_serial_number", "devices", ["serial_number"], unique=False)
        if "ix_devices_os_version" not in device_indexes and "os_version" in {col["name"] for col in inspector.get_columns("devices")}:
            op.create_index("ix_devices_os_version", "devices", ["os_version"], unique=False)
        if "ix_devices_last_contact" not in device_indexes and "last_contact" in {col["name"] for col in inspector.get_columns("devices")}:
            op.create_index("ix_devices_last_contact", "devices", ["last_contact"], unique=False)
        if "ix_devices_username" not in device_indexes and "username" in {col["name"] for col in inspector.get_columns("devices")}:
