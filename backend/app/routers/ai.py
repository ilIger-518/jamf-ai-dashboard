...
async def _policy_spec_from_prompt(message: str) -> dict:
    ...
    content = (
        await complete_chat([{"role": "user", "content": prompt}], temperature=0.1, timeout=30.0)
    ).strip()
...
async def _group_spec_from_prompt(message: str) -> dict:
    ...
    notes = str(parsed.get("notes") or "Generated by Jamf AI Policy and Group Builder").strip()[
        :1024
    ]
...
async def _execute_action_plan(current_user, action: dict) -> str:
    ...
    if action["kind"] == "policy":
        policy_name = ((action["body"].get("policy") or {}).get("general") or {}).get(
            "name", "Unnamed Policy"
        )
...
async def _resolve_knowledge_base_collections(knowledge_base_ids: list[str] | None) -> list[str]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(select(KnowledgeBase).where(KnowledgeBase.id.in_(parsed_ids)))
...
async def chat_stream(current_user: CurrentUser, body: ChatRequest) -> StreamingResponse:
    ...
    async def event_stream():
        try:
            ...
            yield _ndjson_event(
                {"type": "stage", "message": "Searching selected knowledge bases..."}
            )
            rag_context, sources = await _build_rag_context(body.message, body.knowledge_base_ids)
            ...
            if bot_mode == "policy_builder":
                ...
                if pending and _is_cancel_intent(body.message):
                    ...
                    yield _ndjson_event(
                        {"type": "stage", "message": "Executing approved action..."}
                    )
                    approved = _pop_pending_action(current_user.id, session_id_str)
                    if approved is None:
                        reply = "No pending action found."
                    ...
                    plan = await _build_action_plan(
                        body.message, current_user, body.target_server_id
                    )
                    if plan and plan.get("error"):
                        reply = str(plan["error"])
                    elif plan:
